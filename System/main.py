...
# all the dependencies bellow are installed on the production board
import os
import glob
import time
...
getReady = 22 # led indicating to get ready
pose = 23 # led indicating to take the pose !
uploading = 24 # led indicating that the images are being processed...
done = 27 # led indicating that the images are successfully sent !
...
GPIO.setup(lightingTrigger,GPIO.OUT)
...
pygame.display.set_caption("Klych'e")
	...
	if config.capture_count_pics:
		try: # take the photos
			for i in range(1,total_pics+1):
				...
				FILENAMES.append(filename) #save it
